...
async function loadResumes() {
    ...
    resumes.forEach(resume => {
        ...
        resultBox.className = 'mt-2 p-2 bg-gray-100 rounded hidden';
        resultBox.id = `result-${resume.id}`;
        ...
        btn.onclick = async () => {
            ...
            if (response.ok) {
                resultBox.innerHTML = `<strong>Score:</strong> ${match.match_score || 'N/A'}<br>
                                       <strong>Strengths:</strong> ${match.strengths || 'N/A'}<br>
                                       <strong>Gaps:</strong> ${match.gaps || 'N/A'}`;
                resultBox.classList.remove('hidden');

                // Show global results
                document.getElementById('resultsContent').innerHTML = resultBox.innerHTML;
